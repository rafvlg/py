...
# импорт основных библиотек для создания тетриса
import pygame as pg
import random, time, sys
from pygame.locals import *

# Определяем основные параметры: 
# Кадровую частоту fps, высоту и ширину окна программы, 
# размер базового элемента фигур-букв block (20 х 20 пикселей).

fps = 25
window_w, window_h = 600, 500
block, cup_h, cup_w = 20, 20, 10

# Параметры side_freq и down_freq задают время, 
# которое затрачивается на перемещение фигуры в сторону или вниз, 
# если игрок удерживает клавишу нажатой:

side_freq, down_freq = 0.15, 0.1 # передвижение в сторону и вниз

# Для размещения стакана и информационных надписей, 
# а также для конвертации координат нам также понадобятся 
# константы side_margin и top_margin – первая задает 
# дистанцию между правой и левой сторонами окна программы и стаканом; 
# вторая определяет расстояние между верхней границей стакана и окном:
...
lightcolor = ((30, 30, 255), (50, 255, 50), (255, 30, 30), (255, 255, 30)) # светло-синий, светло-зеленый, светло-красный, светло-желтый
...
# размер шаблона должен быть 5 х 5: fig_w, fig_h = 5, 5.
...
# Эта функция отвечает за создание нескольких дополнительных глобальных 
# констант, инициализирует модуль Pygame, рисует стартовое окно игры, 
# вызывает запуск Тетриса runTetris() и в случае необходимости 
...
def main():
    ...
    pg.display.set_caption('Tetris by RafVlg')
    showText('Tetris by RafVlg')
...
# При запуске вызывается функция рисования пустого 
# стакана emptycup(), а возможности движения влево, 
# вправо и вниз устанавливаются на False:
# 	going_down = False
# 	going_left = False
# 	going_right = False
    ...
    while True:
        ...
        if time.time() - last_fall > fall_speed: # свободное падение фигуры
            if not checkPos(cup, fallingFig, adjY = 1): # свободное "приземление" фигуры
                addToCup(cup, fallingFig) # фигура приземлилась добавляем её в содержимое стакана
...
def getNewFig():
    #Возвращает новую фигуру со случайным цветом и угол поворота
...
def drawBlock(block_x, block_y, color, pixelx = None, pixely = None):
    ...
    pg.draw.rect(display_surf, lightcolor[color], (pixelx + 1, pixely + 1, block - 4, block - 4), 0, 3)
...
def drawInfo(points, level):
    ...
    pausedSurf = basic_font.render("Пауза: пробел", True, info_color)
    pauseRect = pausedSurf.get_rect()
    pauseRect.topleft = (window_w - 550, 420)
    display_surf.blit(pausedSurf, pauseRect)
